...
class GlobalVariables:
    def __init__(self):
        self.MYAPP_NAME = ""
        self.MYAPP_VERSION = ""
        self.MYAPP_DESCRIPTION = ""
        self.MYAPP_AUTHOR = ""
        self.MYAPP_AUTHOR_EMAIL = ""
        self.MYAPP_HOMEPAGE = ""
        self.MYAPP_LICENSE = ""
...
class BuildBazelExtension(build_ext.build_ext):
    ...
    def bazel_build(self, ext):
        ...
        bazel_argv = [*get_bazel_target_args("build"), "//pymyapp:stubgen"]
        self.spawn(bazel_argv)
        paths = (
            subprocess.check_output([*get_bazel_target_args("cquery"), "//pymyapp:stubgen"])
            .decode("utf-8")
            .strip()
            .split("\n")
        )
...
setuptools.setup(
    name=config_vars.MYAPP_NAME,
    version=config_vars.MYAPP_VERSION,
    description=config_vars.MYAPP_DESCRIPTION,
    author=config_vars.MYAPP_AUTHOR,
    author_email=config_vars.MYAPP_AUTHOR_EMAIL,
    url=config_vars.MYAPP_HOMEPAGE,
    license=config_vars.MYAPP_LICENSE,
    keywords=["smt", "delta-complete", "qf_lra", "neural network"],
    entry_points={"console_scripts": ["pymyapp=pymyapp.__main__:main"]},
    classifiers=[
    ...
    ext_modules=[BazelExtension("pymyapp._pymyapp", "//pymyapp:_pymyapp")],
    cmdclass={"build_ext": BuildBazelExtension},
    packages=[config_vars.MYAPP_NAME],
